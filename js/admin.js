...
let adminProducts = [];
let adminOrders = [];
let adminResourcePresets = [];
let resourcePresetsLoadedFromTable = false;
let editingProductId = null;
let activeSection = 'orders';

/* ═══ Sections config ═══ */
const ADMIN_SECTIONS = [
  { id: 'products', label: '📦 Produkte', icon: '📦', roles: [ROLE_ADMIN, ROLE_SHOP_MANAGER] },
  { id: 'categories', label: '📂 Kategorien', icon: '📂', roles: [ROLE_ADMIN, ROLE_SHOP_MANAGER] },
  { id: 'resources', label: '💎 Ressourcen', icon: '💎', roles: [ROLE_ADMIN, ROLE_SHOP_MANAGER] },
  { id: 'orders', label: '📋 Bestellungen', icon: '📋', roles: [ROLE_ADMIN, ROLE_SHOP_MANAGER, ROLE_STATUS_ONLY] },
  { id: 'theme', label: '🎨 Theme', icon: '🎨', roles: [ROLE_ADMIN] },
  { id: 'discord', label: '💬 Discord', icon: '💬', roles: [ROLE_ADMIN] },
];

function getAllowedSections() {
    return ADMIN_SECTIONS.filter(s => s.roles.includes(currentRole));
}

function canAccessSection(sectionId) {
    return getAllowedSections().some(s => s.id === sectionId);
}
    ...
    if (sections.length === 0) {
        app.innerHTML = `
          <div class="login-container">
            <div class="login-card fade-in">
              <div class="login-title">
                <h2>⚠️ Kein Zugriff</h2>
                <p>Deine Rolle ist für dieses Admin Panel nicht freigeschaltet.</p>
              </div>
              <button class="btn btn-primary btn-lg btn-full" id="logoutBtn">Abmelden</button>
            </div>
          </div>
        `;
...
async function loadAdminData() {
    ...
    try {
        ...
        if (resourceRes.error) {
            resourcePresetsLoadedFromTable = false;
            const settingsPresets = window.__shopSettings?.resourcePresets;
            adminResourcePresets = Array.isArray(settingsPresets) ? [...settingsPresets] : [];
            console.warn('Resource presets table not available, falling back to shop_settings:', resourceRes.error);
        } else {
            resourcePresetsLoadedFromTable = true;
            adminResourcePresets = resourceRes.data || [];
        }
    } catch (err) {
...
function renderSection(section) {
    ...
    if (!canAccessSection(section)) {
        const fallback = getAllowedSections()[0];
        if (!fallback) {
            content.innerHTML = '<div class="text-muted">Kein Zugriff</div>';
            return;
        }
        activeSection = fallback.id;
        renderSection(activeSection);
        return;
    }
...
function renderAdminProducts(el) {
      ...
      <div class="card-body">
        ...
        <div class="form-row mt-md">
          <div class="form-group" style="flex:1">
            ...
            <div id="pImgPreview" style="margin-top:8px"></div>
          </div>
          <div class="form-group">
    ...
    <div class="card fade-in">
      <div class="card-header">
        ...
        <div class="flex gap-sm">
          <select class="select" id="productCatFilter" style="width:auto; min-width:160px; padding:6px 10px; font-size:0.82rem">
            <option value="all">Alle Kategorien</option>
            ${adminCategories.map(c => `<option value="${c.id}">${escapeHtml(c.icon || '📦')} ${escapeHtml(c.name)}</option>`).join('')}
          </select>
          <button class="btn btn-sm" onclick="loadAdminData().then(()=>renderSection('products'))">🔄</button>
        </div>
      </div>
      <div class="card-body">
  ...
  `;
    ...
    if (imgInput) {
        const updatePreview = () => {
            ...
            if (!preview) return;
            if (url) {
                preview.innerHTML = `<img src="${escapeHtml(url)}" alt="Vorschau" style="max-width:200px; max-height:120px; object-fit:cover; border-radius:var(--radius-sm); border:1px solid var(--glass-border)" onerror="this.parentElement.innerHTML='<span class=\\'text-muted text-sm\\'>⚠️ Bild konnte nicht geladen werden</span>'">`;
            } else {
                preview.innerHTML = '';
            }
        };
    ...
    // Category filter for product table
    const catFilterEl = document.getElementById('productCatFilter');
    if (catFilterEl) {
        catFilterEl.onchange = () => renderProductTable();
    }
...
function renderProductTable() {
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-muted" style="text-align:center">Keine Produkte gefunden</td></tr>';
        return;
    }
...
function fillProductForm(p) {
    ...
    document.getElementById('pImg').dispatchEvent(new Event('input'));
              ...
              <tr>
                ...
                <td>
                  <div class="flex gap-sm items-center">
                    ...
                    ${canDeleteOrders() ? `<button class="btn btn-danger btn-sm" onclick="deleteOrder('${o.id}')">🗑️</button>` : ''}
                  </div>
                </td>
              </tr>
            `;
      ...
      <div class="card-body">
        ...
        <div class="form-row mt-md">
          ...
          <div class="form-group">
            ...
            <input type="color" class="input" id="tText" value="${c.text || '#e2e8f0'}" style="height:44px; padding:6px">
          </div>
          <div class="form-group">
...
async function saveTheme() {
    ...
    settings.title = document.getElementById('tTitle')?.value || 'BulletFarm';
    settings.subtitle = document.getElementById('tSub')?.value || 'By Benzen';
    settings.colors = {
        bg: document.getElementById('tBg')?.value || '#06080c',
        surface: document.getElementById('tSurface')?.value || '#0c1017',
        text: document.getElementById('tText')?.value || '#e2e8f0',
        primary: document.getElementById('tPrimary')?.value || '#60a5fa',
        success: document.getElementById('tSuccess')?.value || '#34d399',
        danger: document.getElementById('tDanger')?.value || '#f87171',
    };
    ...
    try {
        ...
        showToast('🎨 Gespeichert', 'Theme aktualisiert');
    } catch (err) {
        showToast('Fehler', err.message, { type: 'error' });
    }
}
...
function renderAdminDiscord(el) {
    ...
    <div class="card fade-in">
      ...
      <div class="card-body">
        <p class="text-muted text-sm mb-md">Neue Bestellungen werden automatisch an den Discord-Kanal gesendet. Status-Änderungen aktualisieren die bestehende Nachricht live.</p>
        <div class="form-group">
        ...
        <div id="webhookStatus" class="mt-md"></div>
      </div>
    </div>
  `;
}
...
async function saveWebhook() {
    ...
    try {
        ...
        if (error) throw error;
        window.__shopSettings = settings;
        showToast('💾 Gespeichert', 'Webhook URL aktualisiert');
    } catch (err) {
        showToast('Fehler', err.message, { type: 'error' });
    }
}
...
async function testWebhook() {
    ...
    if (!url) {
        statusEl.innerHTML = '<span style="color:var(--danger)">⚠️ Keine URL eingegeben</span>';
        return;
    }

    statusEl.innerHTML = '<span class="text-muted">Sende Test…</span>';
        ...
        const response = await fetch(url + '?wait=true', {
            ...
            body: JSON.stringify({
                embeds: [{
                    ...
                    description: 'Der BulletFarm Shop Webhook funktioniert.',
                    color: 0x34D399,
                    footer: { text: `BulletFarm Shop • ${new Date().toLocaleDateString('de-DE')}` },
...
function getResourcePresets() {
    if (resourcePresetsLoadedFromTable) {
        return adminResourcePresets;
    }
...
async function addResourcePreset() {
    ...
    try {
        ...
        if (resourcePresetsLoadedFromTable) {
            const nextSort = presets.reduce((max, p) => Math.max(max, Number(p.sort_order) || 0), 0) + 1;
            const { data, error } = await sb
                .from('resource_presets')
                .insert({ name, color, sort_order: nextSort })
                .select()
                .single();
            if (error) throw error;
            adminResourcePresets.push(data);
        } else {
            const settings = window.__shopSettings || {};
            settings.resourcePresets = [...presets, { name, color }];
            const { error } = await sb.from('shop_settings').update({ data: settings }).eq('id', 1);
            if (error) throw error;
            window.__shopSettings = settings;
            adminResourcePresets = settings.resourcePresets;
        }
...
async function deleteResourcePreset(index) {
    ...
    try {
        ...
        if (resourcePresetsLoadedFromTable && preset.id) {
            const { error } = await sb.from('resource_presets').delete().eq('id', preset.id);
            if (error) throw error;
            adminResourcePresets = adminResourcePresets.filter(p => p.id !== preset.id);
        } else {
            const settings = window.__shopSettings || {};
            settings.resourcePresets = presets.filter((_, i) => i !== index);
            const { error } = await sb.from('shop_settings').update({ data: settings }).eq('id', 1);
            if (error) throw error;
            window.__shopSettings = settings;
            adminResourcePresets = settings.resourcePresets;
